...
public final class WormHomeView extends FrameLayout {
    ...
    /*
     * ORIGINAL GAPS FROM home-target.png (reference 691x1536):
     ...
     * dock -> bottom            = 26 px
     *
     * Geometry is uniformly scaled from reference width.
     * Extra display height remains system wallpaper.
     */

    /*
     * home-target.png reference canvas:
     * 691 x 1536 px
     *
     * Measured lower-home geometry from the reference:
     *
     * MAIN OUTER:
     * x = 67
     * y = 639
     ...
     * h = 132
     *
     * The implementation scales this canvas UNIFORMLY by screen width.
     * Extra vertical space stays wallpaper.
     */

    private static final float REF_W = 691f;
    private static final float REF_H = 1536f;

    private static final float MAIN_X = 67f;
    private static final float MAIN_Y = 639f;
    ...
    private static final float DOCK_Y = 1378f;
    private static final float DOCK_W = 555f;
    private static final float DOCK_H = 132f;

    // Sampled / matched from home-target.png.
    private static final int COLOR_OUTER = Color.rgb(21, 21, 21);   // #151515
    private static final int COLOR_PANEL = Color.rgb(35, 35, 35);   // #232323
    private static final int COLOR_DOCK = Color.rgb(21, 21, 21);    // #151515
    private static final int COLOR_BANNER = Color.rgb(95, 103, 106);// #5F676A
    private static final int COLOR_TEXT = Color.rgb(248, 248, 248);  // #F8F8F8
    private static final int COLOR_SUBTEXT = Color.rgb(235, 235, 235);

    private static final int COLOR_EDGE =
    ...
    private void rebuild() {
        ...
        // No bundled / preset wallpaper.
        // The Activity window shows the phone's current system wallpaper.
        addTargetHome();
    ...
    private void addTargetHome() {
        ...
        // If display is taller than the reference aspect ratio,
        // we preserve width-derived scale and leave the extra height as wallpaper.
        final int mainX = px(MAIN_X, scale);
        final int mainW = px(MAIN_W, scale);
        final int mainH = px(MAIN_H, scale);

        // Anchor lower composition to bottom using target's bottom distances.
        final int refMainBottomGap =
                Math.round(REF_H - (MAIN_Y + MAIN_H));
        final int mainBottomGap = px(refMainBottomGap, scale);
        ...
        dockLp.leftMargin = px(DOCK_X, scale);

        int refDockBottomGap =
                Math.round(REF_H - (DOCK_Y + DOCK_H));
        dockLp.bottomMargin = px(refDockBottomGap, scale);
    ...
    private View buildDock(
        ...
        for (int i = 0; i < count; i++) {
            ...
            // PATCH-001 does not redefine icon rendering.
            // PATCH-002 will handle optical normalization.
    ...
    private GridLayout.LayoutParams gridCellParams(
            float scale
    ) {
    ...
    private float spFromRef(
        ...
        return (referencePx * scale) / density;
    }
