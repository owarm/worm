...
NGINX_CONF="/etc/nginx/sites-enabled/worm-system-public-guards.conf"
...
test -f "$SHA"
test -f "$NGINX_CONF"
grep -q 'server_name apps.coffee.pm releases.coffee.pm ota.coffee.pm' "$NGINX_CONF"
test -d "$DOCROOT"

mkdir -p "$PUBLIC_DIR"
cp "$APK" "$PUBLIC_DIR/MachinaLLM-0.1.0.apk"
...
<head>
  ...
  <title>MachinaLLM 0.1.0</title>
</head>
<body>
  <h1>MachinaLLM</h1>
  <p>Version 0.1.0</p>
  <p><a href="MachinaLLM-0.1.0.apk">Download APK</a></p>
...
curl --fail --location -o /tmp/MachinaLLM-download-test.apk "$URL"
expected="$(sha256sum "$APK" | awk '{ print $1 }')"
actual="$(sha256sum /tmp/MachinaLLM-download-test.apk | awk '{ print $1 }')"
rm -f /tmp/MachinaLLM-download-test.apk
