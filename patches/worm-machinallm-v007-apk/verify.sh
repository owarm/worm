...
if rg -n --hidden -S 'OPENAI_API_KEY|API_KEY|SECRET|TOKEN|PASSWORD|Bearer|sk-|OkHttp|Retrofit|WebView|analytics|telemetry|crashlytics|firebase' "$PROJECT_DIR/app/src/main" "$PROJECT_DIR/app/build.gradle.kts" "$PROJECT_DIR/gradle.properties" "$PROJECT_DIR/settings.gradle.kts" "$PROJECT_DIR/build.gradle.kts" >/tmp/machinallm-v007-security.txt; then
    sed 's/:.*/: [REDACTED]/' /tmp/machinallm-v007-security.txt
...
if rg -n 'Relaxed|#[0-9A-Fa-f]{6,8}' "$PROJECT_DIR/app/src/main" >/tmp/machinallm-v007-colors.txt; then
    cat /tmp/machinallm-v007-colors.txt
...
echo "[INFO] versionCode: 1"
echo "[INFO] versionName: 0.1.0"
echo "[INFO] minSdk: 31"
